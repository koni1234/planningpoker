...
import { ALL_COLORS, FLEX_ALIGN, FLEX_JUSTIFY, FLEX_WRAP, GRID_GUTTERS, TEXT_SIZES } from '../ui.enums';
        ...
        <template v-for="(user, index) in props.game.users" :key="index">
            <pp-grid-item class="vote-wrapper">
                ...
                <div class="vote" :class="{ 'has-voted': !!user.vote || props.game.closed }">
                    <pp-text v-if="props.game.closed" :size="TEXT_SIZES.SIZE_24" :color="ALL_COLORS.WHITE">{{
                        user.vote || '??'
                    }}</pp-text>
                </div>
            </pp-grid-item>
